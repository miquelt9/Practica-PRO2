...
Usuario::Usuario(const Usuario& u)
{
  ...
  ses_completadas = u.ses_completadas;
}
  ...
  const Cjt_Sesiones& ses, Cjt_Cursos& cursos)
{
  ...
  if (r == 1)
  {
    ...
    list<string> t;
    ses.agregar_hijos(cursos.sesion_problema(curso ,p), p, t);
    agregar_enviables(t, cursos, ses);
    enviables.erase(p);
  }
  if (curso_terminado())
  {
    cursos.reducir_registrados(curso);
    cursos.aumentar_completados(curso, id);
    terminar_curso();
  }
}

void Usuario::agregar_enviables(list<string>& ls, const Cjt_Cursos& cursos,
const Cjt_Sesiones& sesiones)
{
  ...
  while (it != ls.end())
  {
    ...
    else //Se han resuelto, vamos a buscar a sus hijos
    {
      sesiones.agregar_hijos(cursos.sesion_problema(curso, *it), *it, ls);
    }
    ++it;
  }
}
...
void Usuario::sesiones_completadas(vector<string>& sesiones)
{
  sesiones = ses_completadas;
}
