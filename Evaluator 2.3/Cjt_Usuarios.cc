...
void Cjt_Usuarios::baja_usuario(const string& u, Cjt_Cursos& cursos)
{
  ...
  if (c != 0) cursos.reducir_registrados(c);
  usuarios.erase(u);
}
  ...
  const Cjt_Sesiones& sesiones)
{
  ...
  for (int i = 0; i < (int)ses_completadas.size(); ++i) ls.remove(ses_completadas[i]);

  list<string> primeros_problemas;
  sesiones.primeros_problemas(ls, primeros_problemas);
  user.agregar_enviables(primeros_problemas, cursos, sesiones);

  if (user.curso_terminado())
  {
    cursos.reducir_registrados(user.curso_usuario());
