...
public:
  ...
  */
  ...
  /** @brief Primeros problemas
      ...
      \pre  Las sesioes contenidas en ls existen
      \post Al parámetro primeros pronlemas son añadidos los primeros problemas
            de cada sesions de ls
  */
  void primeros_problemas(const list<string>& ls, list<string>& primeros_problemas) const;

  /** @brief Problemas sesion

      \pre  La sesion s existe
      \post Se añaden los problemas de s a m
  */
  void problemas_sesion(map<string,string>& m, const string& s) const;

  /** @brief Numero problemas

      \pre  La sesion s existe
      \post Devuelve el numero de problemas que tiene s
  */
  int numero_problemas(const string& s) const;
...
private:
  /**@brief Mapa de sesiones de acceso por id*/
