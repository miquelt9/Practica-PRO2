...
// Pre:  el map prob está vacio
// Post: a prob se le añade como "key" el valor del node a i como "value" los
//       los hijos de a, así para todos los nodos de a
...
void Sesion::problemas_sesion(map<string,string>& m) const
{
  for (map<string,pair<string,string>>::const_iterator it = problemas.begin();
  it != problemas.end(); ++it)
  {
    m.insert(make_pair(it->first, id));
  }
}
