...
int main()
{
    ...
    while (comando != "fin")
    {
      ...
      else if (comando == "sesion_problema" or comando == "sp")
      {
        ...
        if (cursos.existe_curso(c))
        {
          if(coleccion_problemas.existe_problema(p))
          {
            ...
            cursos.sesiones_curso(c, sesiones_curso);
            string sesion = repositorio_sesiones.sesion_problema(sesiones_curso, p);
      ...
      {
        cout << endl << "error: comando no valido" << endl;
      }
      cin >> comando;
    }
}
