...
{
  int c = curso_usuario(u);
  cerr << "Baja del usuario " << u << " del curso " << c << endl;
  usuarios.erase(u);
  cerr << "usuario borrado" << endl;
  cursos.baja_registrados(c);
  cerr << "baja registrado" << endl;
  cursos.baja_completados(c);
  cerr << "baja completado" << endl;
}
